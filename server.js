...
function getDeviceIds(row) {
  if (Array.isArray(row.device_ids) && row.device_ids.length) return row.device_ids;
  if (row.device_id) return [row.device_id];
  return [];
}

const MAX_DEVICES = 2; // extensão (PC) + app (celular)

function tokenPayload(row) {
  const expired = isExpired(row);
  const status = row.status === "revoked" ? "revoked" : expired ? "expired" : row.status;
  const devices = getDeviceIds(row);
  return {
    id: row.id,
    token: row.token,
    status,
...
function doValidate(token, deviceId) {
  ...
  // Multi-device: até MAX_DEVICES (PC extensão + celular app)
  let devices = getDeviceIds(row);
  const alreadyBound = devices.includes(cleanDevice);
  if (!alreadyBound) {
    if (devices.length >= MAX_DEVICES) {
      return {
        ...
        body: {
          ok: true,
          valid: false,
          error: "Limite de dispositivos atingido (máx. " + MAX_DEVICES + ": PC + celular). Desvincule um no painel admin."
        }
      };
    }
    devices = devices.concat([cleanDevice]);
    row.device_ids = devices;
    row.device_id = devices[0]; // compatibilidade
    if (!row.activated_at) row.activated_at = nowISO();
  }
  ...
  return {
    ...
    body: {
      ...
      valid: true,
      expiresAt: row.expires_at || null,
      firstActivation: !alreadyBound && devices.length === 1,
      deviceCount: devices.length,
      maxDevices: MAX_DEVICES,
      message: alreadyBound
        ? "Token válido"
        : "Dispositivo autorizado (" + devices.length + "/" + MAX_DEVICES + ")"
    }
  };
}
...
const adminPath = fs.existsSync(path.join(__dirname, "admin")) ? path.join(__dirname, "admin") : path.join(__dirname, "..", "admin");
if (fs.existsSync(adminPath)) app.use("/admin", express.static(adminPath));

app.get("/", (_req, res) => {
  res.json({ name: "Live Max License Server", version: "2.1", admin: "/admin/", health: "ok", maxDevices: 2 });
});
...
app.patch("/api/admin/tokens/:id", requireAdmin, (req, res) => {
  ...
  if (unbind === true) { row.device_id = null; row.device_ids = []; row.activated_at = null; }
  saveDb(db);
...
app.get("/api/admin/stats", requireAdmin, (_req, res) => {
  ...
  const bound = db.tokens.filter((t) => getDeviceIds(t).length > 0).length;
